    ...
    find "$libdir" -name '*.jd' | LC_ALL=C sort | while IFS= read -r f; do
      ...
      JD_REL="$rel" awk '
        ...
        END{
          ...
          printf "%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n", key, name, kind, purpose, tags, rel, links, usew, notw, danger, effects, requires
        }
      ' "$f" >> "$out.tmp"
    done
...
search query kind="" num="5":
    ...
    kind={{quote(kind)}}; num={{quote(num)}}
    # user input crosses into awk via ENVIRON (no -v backslash-escape processing)
    export JD_QUERY={{quote(query)}} JD_KIND="$kind" JD_NUM="$num" JD_BASE="$raw_base"
...
get ref only="":
    ...
    printf '%s\n' "$body" | awk -v fmt="$format" '
      ...
      function flush(  i,isjust,injust,buf){
        if (bn==0) return
        isjust=0
