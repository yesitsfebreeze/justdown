...
func toolCatalogue() []map[string]any {
	...
	return []map[string]any{
		{
			"name":        "search",
			"description": "Rank library .jd files by need (graph-aware: name/use_when > tags > prose; not_when vetoes), then append files the rank missed whose file name (fuzzy) or content matches every term at score 0 — the editor's search semantics. Returns the best matches with purpose, kind, source tier and safety.",
