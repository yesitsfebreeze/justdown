...
func TestFlagsOverlappingVariants(t *testing.T) {
	errs := PlatformErrors(block("[unix, wsl]\nr:\n  a\n[wsl]\nr:\n  b"))
	if len(errs) != 1 {
		t.Fatalf("%v", errs)
	}
...
func TestIgnoresFilesWithoutTheConvention(t *testing.T) {
	if errs := PlatformErrors(block("a:\n  one\nb:\n  two")); len(errs) != 0 {
		t.Fatalf("%v", errs)
	}
}

func TestRecipeNameDetectsHeadersOnly(t *testing.T) {
	if n, ok := RecipeName("open target:"); !ok || n != "open" {
		t.Fatalf("%q %v", n, ok)
	}
	if n, ok := RecipeName("check host count=\"5\":"); !ok || n != "check" {
		t.Fatalf("%q %v", n, ok)
	}
	for _, bad := range []string{"  xdg-open x", "# comment", "[unix]", "x := 1"} {
		if _, ok := RecipeName(bad); ok {
...
func TestLintNodeNoFrontmatterIsOnlyFinding(t *testing.T) {
	n := Parse("library/x/foo.jd", "no frontmatter here\n")
	f := LintNode(&n, "no frontmatter here\n")
