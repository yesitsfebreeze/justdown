...
// Size is a terminal cell size, sent down Options.Resize whenever the
// embedder's own terminal (real or virtual) resizes.
type Size struct{ W, H int }

// Options configures an embedded Run. No file descriptor or raw-mode
// assumption lives here — the caller owns the real terminal (cmd/jd's own
// wrapper: MakeRaw + SIGWINCH) or a virtual one (an embedder like burrito,
// piping a pane's already-raw keystrokes straight through) and just hands
// over byte streams plus an initial size.
type Options struct {
	Roots      []string
	ProjectDir string
	BeltRows   func() []justdown.Row // optional: cached remote/plugin capability rows
	Env        []string              // optional: for color-profile detection (os.Environ())

	Stdin         io.Reader
	Stdout        io.Writer
	Width, Height int
	Resize        <-chan Size // optional; nil if the embedder never resizes it
}

// Run drives the full-screen jd editor until the user quits (ctrl+q) or Stdin
// closes/errors. It returns nil on either clean exit — there is no separate
// error path once Options are valid, since Run does no fd/terminal-mode setup
// of its own.
func Run(opts Options) error {
	a := &app{
		lib:   newLibrary(opts.ProjectDir, opts.BeltRows, opts.Roots),
		ed:    newEditor(),
		fnd:   &finder{},
		pop:   &linkpop{},
		in:    opts.Stdin,
	...
	fmt.Fprint(a.out, "\x1b[?1049h\x1b[?25l\x1b[?7l\x1b[?2004h\x1b[>1u\x1b[>4;1m")
	defer fmt.Fprint(a.out, "\x1b[>4;0m\x1b[<u\x1b[?2004l\x1b[?7h\x1b[?25h\x1b[?1049l")

	a.cols, a.rows = opts.Width, opts.Height
	a.applyLayout()
	...
	r := uv.NewTerminalRenderer(a.out, opts.Env)
	...
	resize := opts.Resize
	relayout := func(sz Size) {
		if sz.W == a.cols && sz.H == a.rows {
			return
		}
		a.cols, a.rows = sz.W, sz.H
		a.applyLayout()
		r.Resize(sz.W, sz.H)
		r.Erase()
	}
	...
	for {
		...
		case sz, ok := <-resize:
			if !ok {
				resize = nil // closed: stop selecting on it
				continue
			}
			relayout(sz)
			pending = true
